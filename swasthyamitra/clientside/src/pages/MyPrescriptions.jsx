  ...
  const getPrescriptions = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Could not load e-prescriptions");
      }
    } catch (error) {
      console.log(error);
      toast.error(error.message || "Could not load e-prescriptions");
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-sm text-zinc-500 mt-1 mb-4">
        E-prescriptions issued by doctors after your appointments.
      </p>
      {prescriptions.length === 0 ? (
        <div className="py-12 text-center text-zinc-500 border border-dashed border-zinc-200 rounded-lg">
          <p className="font-medium text-zinc-600">No e-prescriptions yet</p>
          <p className="text-sm mt-1">
            E-prescriptions will appear here after your doctor adds them following an appointment.
          </p>
        </div>
      ) : (
